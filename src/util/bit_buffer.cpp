...
BitBuffer::~BitBuffer() {
    delete[] reinterpret_cast<uint8_t *>(_buffer);
}

void BitBuffer::write(BitBufferhandle handle, const void *data, size_t size) {
    if (handle.size + size * 8 > handle.capacity || size == 0) return;
...
bool BitBuffer::read(BitBufferhandle handle, void *data) const {
    if (handle.size + 8 > handle.capacity) return false;
...
Option<void *> BitBuffer::read(BitBufferhandle handle) const {
    if (handle.size >= handle.capacity) return Option<void *>::none();

    size_t byteOffset = handle.size / 8;
    if (byteOffset >= byteSize()) return Option<void *>::none();
...
template <typename T>
void BitBuffer::write(BitBufferhandle handle, T value) {
    write(handle, &value, sizeof(T));
}

template <typename T>
Option<T> BitBuffer::read(BitBufferhandle handle) const {
    T value;
    if (!read(handle, &value)) return Option<T>::none();
