...
CANBus::CANBus(CANBaudRate baudRate, gpio_num_t txPin, gpio_num_t rxPin)
    : _baudRate(baudRate), _txPin(txPin), _rxPin(rxPin),
      _buffer(BitBuffer::empty()), _nextBitOffset(0) {}
...
void CANBus::initialize() {
  // Configure TWAI (ESP32 CAN) driver.
    ...
    for (auto &msg : _messages) {
      if (msg.id == twaiMsg.identifier) {
        ...
        for (size_t i = 0;
          ...
          setSignal<uint8_t>(signal, twaiMsg.data[i]);
        }
