...
const uint8_t LoRaPacket::END_BYTE = 0xA1;
const size_t LoRaPacket::MAX_PAYLOAD = 248;

LoRaPacket::LoRaPacket(uint8_t msg_type, uint8_t options, uint16_t seq_num,
...
LoRaPacket::LoRaPacket()
    : _msgType(0), _options{.raw = 0}, _seqNum(0), _payloadSize(0), _payload() {}

void LoRaPacket::changePayloadSize(size_t new_val) { _payloadSize = new_val; }
