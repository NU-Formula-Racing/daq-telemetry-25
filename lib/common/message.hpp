...
class LoRaPacket {
 public:
  static const uint8_t END_BYTE;    // Magic byte to check if the packet is indeed ours, in the end
  static const size_t MAX_PAYLOAD;  // Hard limit, if we changed the MAX_PAYLOAD
  // we would have packets bigger than 255 (LoRa max) due to the headers

  enum : uint8_t { OPT_NEEDS_ACK = 1 << 0, OPT_IS_ACK = 1 << 1 };
